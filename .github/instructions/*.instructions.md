## 1. Repository Scope
  ...
  **Do not introduce** new public apps, dashboards, or internal
  ...
  up)
  - Root `Cargo.toml`:
    - `[profile.release]` is generally defined at the workspace root.
      Member packages may define their own `[profile.*]` sections if required for Solana/SBF/Anchor compatibility (e.g., for `programs/token_2022`).
  ...
  **Copilot must:**
  ...
  - `.github/workflows/guard.yml`
    - Enforces:
      - No forbidden paths (`apps/*`; the only allowed oracle is at `oracles/x402-switchboard/`)
  ...
  **Copilot must:**
  ...
  - Never change CI paths blindly.
    Respect `defaults.run.working-directory`. E.g., if
    ```yaml
    working-directory: programs/token_2022
    ```
    is set, then:
    - Artifacts are at `target/deploy/token_2022.so` and `target/idl/token_2022.json` (no extra `programs/token_2022/` prefix).
  ...
  **Copilot must:**
  ...
  3. **Are CI paths correct?**
     - Workflows use the right artifact paths given their `working-directory`.
