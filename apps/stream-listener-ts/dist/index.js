...
const connection = new Connection(env.RPC_URL, {
    ...
    commitment: env.STREAM_COMMITMENT,
});
...
async function main() {
    ...
    const subId = await connection.onLogs(PROGRAM_ID, (logs, ctx) => {
        if (logs.err)
            return;
    ...
    try {
        ...
        const accId = await connection.onAccountChange(protocolPda, (info, ctx) => {
            try {
                let state = null;
                try {
                    state = program.coder.accounts.decode('protocolState', info.data);
                }
                catch { }
                if (!state) {
                    try {
        ...
        if (env.MINT_PUBKEY && env.STREAM_CHANNELS && env.STREAM_CHANNELS.length > 0) {
            ...
            for (const channel of channels) {
                try {
                    ...
                    const [pda] = PublicKey.findProgramAddressSync([Buffer.from('channel_state'), mint.toBuffer(), streamerKey.toBuffer()], PROGRAM_ID);
                    const id = await connection.onAccountChange(pda, (info, ctx) => {
                        const payload = {
                            ts: new Date().toISOString(),
                            slot: ctx.slot,
                            type: 'channel_update',
