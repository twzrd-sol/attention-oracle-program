...
// API base URL - uses gateway URL from environment or falls back to relative paths
...
export async function getVerificationStatus(wallet: string): Promise<VerificationStatus> {
  try {
    const response = await fetch(`${API_BASE_URL}/api/verification-status?wallet=${encodeURIComponent(wallet)}`);
...
export async function requestClaimTransaction(
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/claim-cls`, {
      method: 'POST',
      headers: {
...
export async function bindWalletWithTwitch(token: string, wallet: string): Promise<BindingResponse> {
  const response = await fetch(`${API_BASE_URL}/api/bindings/bind-wallet`, {
    method: 'POST',
...
export async function fetchBoundWallet(token: string): Promise<BoundWalletResponse> {
  const response = await fetch(`${API_BASE_URL}/api/bindings/bound-wallet`, {
...
export async function getEpochs(limit = 50, offset = 0): Promise<EpochsResponse> {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/epochs?${params.toString()}`);
...
export async function getEpoch(epochId: number): Promise<Epoch> {
  try {
    const response = await fetch(`${API_BASE_URL}/api/epochs/${epochId}`);
...
export async function getClaimHistory(
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/claims/history?${params.toString()}`);
