...
use crate::state::{
    PriceFeedState, ProtocolState, DISC_PRICE_FEED_STATE, PRICE_FEED_SEED, PROTOCOL_STATE_SEED,
};
...
pub fn initialize_price_feed(
    ...
    let (expected_pda, bump) = pubkey::find_program_address(&[PRICE_FEED_SEED, &label], program_id);
...
pub fn update_price(
    ...
    let pda =
        pubkey::create_program_address(&[PRICE_FEED_SEED, &feed.label, &[feed.bump]], program_id)?;
