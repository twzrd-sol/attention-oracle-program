...
pub fn harvest_fees(
    ...
    // Owner check: protocol_state must be owned by this program
    if protocol_state.owner() != program_id {
    ...
    // Instruction data: [26, 3, num_token_accounts]
    // Token-2022 requires the count byte to parse the instruction.
    // Accounts: [mint(W), destination(W), authority(S), ...sources(W)]
    let num_sources = remaining.len() as u8;
    let cpi_ix_data: [u8; 3] = [
        TRANSFER_FEE_EXTENSION,
        WITHDRAW_FROM_ACCOUNTS_SUB_IX,
        num_sources,
    ];
