...
pub fn realloc_legacy_protocol(
    ...
    let (expected_legacy, _) = pubkey::find_program_address(&[b"protocol", mint_key], program_id);
...
pub fn admin_fix_ccm_authority(
    ...
    pinocchio::cpi::slice_invoke_signed(&instruction, &[mint, protocol_state], &[signer])?;
...
pub fn create_channel_config_v2(
    ...
    let (expected_pda, bump) = pubkey::find_program_address(&[SEED, &mint, &subject], program_id);
