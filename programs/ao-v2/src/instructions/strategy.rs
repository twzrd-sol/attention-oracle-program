...
use crate::klend;
use crate::state::{
    MarketVault, ProtocolState, StrategyVault, DISC_STRATEGY_VAULT, MARKET_VAULT_SEED,
    STRATEGY_VAULT_SEED,
};
...
const SYSVAR_INSTRUCTIONS_ID: Pubkey = [
    0x06, 0xa7, 0xd5, 0x17, 0x18, 0x7b, 0xd1, 0x60, 0x35, 0xfe, 0x6b, 0x0a, 0x62, 0xda, 0x8a, 0x09,
    0x1f, 0x4d, 0x31, 0x07, 0x08, 0xb4, 0xc2, 0xfb, 0x2f, 0x7e, 0x44, 0x73, 0x00, 0x00, 0x00, 0x00,
];
...
pub fn initialize_strategy_vault(
    ...
    let ps_pda = pubkey::create_program_address(&[b"protocol_state", &[ps.bump]], program_id)?;
    ...
    let (expected_sv_pda, sv_bump) =
        pubkey::find_program_address(&[STRATEGY_VAULT_SEED, market_vault_acc.key()], program_id);
...
fn validate_reserve_data(
    ...
    if !pubkey::pubkey_eq(deposit_mint.key(), &sv.deposit_mint)
        || !pubkey::pubkey_eq(&r_liq_mint, deposit_mint.key())
    {
        return Err(OracleError::InvalidMint.into());
    }
    ...
    let expected_lma =
        klend::derive_lending_market_authority(&sv.klend_lending_market, klend_program.key());
