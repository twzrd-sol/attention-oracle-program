...
pub use instructions::{admin, global, governance, stream, vault};
...
// ============================================================================
// SECURITY.TXT (Neodyme standard — scanned by explorers / audit tools)
// ============================================================================

// The solana-security-txt crate requires std. For no_std Pinocchio programs,
// we embed the same magic-delimited format directly. Tools like SolanaFM and
// Sec3 scan the binary for the =BEGIN/END= markers.
#[cfg_attr(target_os = "solana", link_section = ".security.txt")]
#[allow(dead_code)]
#[no_mangle]
pub static security_txt: [u8; 389] = *b"\
=======BEGIN SECURITY.TXT V1=======\0\
name\0Liquid Attention Protocol\0\
project_url\0https://twzrd.xyz\0\
contacts\0email:security@twzrd.xyz\0\
policy\0https://github.com/twzrd-sol/attention-oracle-program/blob/main/SECURITY.md\0\
preferred_languages\0en\0\
source_code\0https://github.com/twzrd-sol/attention-oracle-program/tree/main/programs/ao-v2\0\
source_revision\0\0\
auditors\0\0\
...
pub fn process_instruction(
    ...
    match disc {
        ...
        // ==================================================================
        // STREAM (vLOFI mint claims)
        // ==================================================================

        // publish_stream_root
        [0x2f, 0xfa, 0x4d, 0xc7, 0x3e, 0x90, 0x82, 0x4c] => {
            stream::publish_stream_root(program_id, accounts, ix_data)
        }

        // claim_stream
        [0x9d, 0xf7, 0xa4, 0xe2, 0xf0, 0x9e, 0xb7, 0x24] => {
            stream::claim_stream(program_id, accounts, ix_data)
        }

        // claim_stream_sponsored
        [0xeb, 0xb1, 0x48, 0xf6, 0xff, 0xf3, 0xea, 0xbc] => {
            stream::claim_stream_sponsored(program_id, accounts, ix_data)
        }
        ...
        // compound_stake — native compound (no CPI boundary)
        // SHA-256("global:compound_stake")[..8]
        [0x04, 0x26, 0x63, 0x4b, 0x3f, 0x76, 0xad, 0x77] => {
            compound::compound_stake(program_id, accounts, ix_data)
        }
