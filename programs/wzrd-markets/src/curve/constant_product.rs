//! The Uniswap invariant `ConstantProductCurve`.
//!
//! Vendored from `raydium-io/raydium-cp-swap`
//! (`libraries/src/curve/constant_product.rs`, Apache-2.0). Vendored 2026-06-21.
//!
//! Hardening applied during the port (the only deviation from upstream):
//!   - `swap_base_input_without_fees`, `swap_base_output_without_fees`, and the
//!     intermediate steps of `lp_tokens_to_trading_tokens` returned via
//!     `.unwrap()` upstream. Here every arithmetic step is checked and the
//!     functions return `Option<_>`. On-chain code MUST NOT panic on bad input;
//!     callers map `None` to a program error. The exact math (the
//!     `(x+dx)(y-dy)=xy` invariant, ceil-div for base-output) is preserved.

use crate::curve::calculator::{RoundDirection, TradingTokenResult};
use crate::curve::checked_math::CheckedCeilDiv;

/// `ConstantProductCurve` — `x * y = k` swap and LP conversion math.
...
impl ConstantProductCurve {
    ...
    ///
    /// Returns `None` instead of panicking if any intermediate step overflows
    /// or divides by zero (hardened vs the upstream `.unwrap()` version).
    pub fn swap_base_input_without_fees(
        ...
        let output_amount = numerator.checked_div(denominator)?;
        Some(output_amount)
    }

    /// Inverse of [`swap_base_input_without_fees`]: given a desired output, the
    /// required input. Uses ceiling division so the pool never gives out more
    /// than the invariant allows.
    ///
    /// Returns `None` instead of panicking on overflow / divide-by-zero
    /// (hardened vs the upstream `.unwrap()` version). In particular,
    /// `output_amount >= output_vault_amount` yields `None` (the pool cannot
    /// fill an order that would drain or over-drain the output reserve).
    pub fn swap_base_output_without_fees(
        ...
        let input_amount = numerator.checked_ceil_div(denominator)?;
        Some(input_amount)
    }
