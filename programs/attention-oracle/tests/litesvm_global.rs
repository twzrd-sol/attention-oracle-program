...
fn test_mainnet_global_root_config_pda() {
    ...
    // Uses mainnet program ID explicitly (not program_id() which is devnet).
    let mainnet_program: Pubkey = "GnGzNdsQMxMpJfMeqnkGPsvHm8kwaDidiKjNU2dCVZop"
        .parse()
        .unwrap();
    ...
    let (global_root_config, bump) =
        Pubkey::find_program_address(&[GLOBAL_ROOT_SEED, ccm_mint.as_ref()], &mainnet_program);
...
fn test_mainnet_protocol_state_pda() {
    let (protocol_state, _bump) = derive_protocol_state();

    // Expected PDA derived from seeds [b"protocol_state"] with devnet program ID
    let expected: Pubkey = "41cBx7vxe2Tqz4hx23T2CBYS8fhEufyyxqRpxNX4BUVy"
        .parse()
        .unwrap();

    assert_eq!(protocol_state, expected, "ProtocolState PDA mismatch");

    println!("  ProtocolState PDA: {} — matches expected", protocol_state);
}
