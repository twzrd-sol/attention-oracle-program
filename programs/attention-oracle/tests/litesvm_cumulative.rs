...
fn address_from_legacy(pubkey: &Pubkey) -> Address {
    Address::from(pubkey.to_bytes())
}

fn legacy_from_address(address: &Address) -> Pubkey {
    Pubkey::new_from_array(address.to_bytes())
}

fn legacy_from_signer(signer: &Keypair) -> Pubkey {
    legacy_from_address(&signer.pubkey())
}

fn convert_instruction(ix: &Instruction) -> solana_instruction::Instruction {
...
fn test_pda_derivation() {
    ...
    let (protocol_state, bump) = derive_protocol_state();
    ...
    // Case insensitive
    let subject_id_upper = derive_subject_id("TEST_CHANNEL");
...
fn test_initialize_protocol() {
    ...
    // Derive PDAs
    let (protocol_state, _) = derive_protocol_state();
    ...
    let ix = Instruction {
        ...
        accounts: vec![
            AccountMeta::new(legacy_from_signer(&admin), true),
            AccountMeta::new(protocol_state, false),
            AccountMeta::new(legacy_from_signer(&mint), true),
            AccountMeta::new_readonly(system_program::ID, false),
        ],
        data,
    };

    match send_legacy_tx(&mut svm, &[&admin, &mint], &admin, &[ix]) {
...
fn test_chaos_channel_case_sensitivity() {
    let channel_lower = "test_channel";
    let channel_upper = "TEST_CHANNEL";
    let channel_mixed = "Test_Channel";
...
fn test_chaos_treasury_ata_ownership() {
    let mint = Pubkey::new_unique();
    let (protocol_state_pda, _bump) = derive_protocol_state();
