...
//!
//! Run with: `cargo test --package attention-oracle-token-2022 --test litesvm_staking --features phase2`
//!
//! Tests cover:
...
use token_2022::{
    ...
    BPS_DENOMINATOR, CHANNEL_STAKE_POOL_SEED, CUMULATIVE_ROOT_HISTORY, MAX_APR_BPS,
    MIN_RUNWAY_SLOTS, REWARD_PRECISION, SLOTS_PER_DAY, SLOTS_PER_YEAR,
};
...
// ============================================================================
// Part 10: Set Reward Rate Tests (LiteSVM)
// ============================================================================

/// Helper: set up LiteSVM with protocol_state, channel_config, stake_pool, vault
/// for set_reward_rate tests.
fn setup_reward_rate_env(
    total_staked: u64,
    vault_balance: u64,
) -> (LiteSVM, Keypair, Pubkey, Pubkey, Pubkey) {
    let mut svm = LiteSVM::new();

    if load_program(&mut svm).is_err() {
        panic!("Program not compiled");
    }
    ...
    let stake_pool_data = ChannelStakePool {
        ...
        staker_count: if total_staked > 0 { 1 } else { 0 },
        acc_reward_per_share: 0,
        last_reward_slot: 0,
        reward_per_slot: 0,
        is_shutdown: false,
    ...
    (svm, admin, channel_config, stake_pool, vault)
}
...
#[test]
fn test_set_reward_rate_happy_path() {
    let total_staked = 100_000_000_000u64; // 100 CCM
    let reward_reserves = 50_000_000_000u64; // 50 CCM reserves
    let vault_balance = total_staked + reward_reserves;

    let (mut svm, admin, channel_config, stake_pool, vault) =
        setup_reward_rate_env(total_staked, vault_balance);

    // 10% APR
    let new_rate = (total_staked as u128)
    ...
    assert!(new_rate > 0, "Rate should be non-zero");
    assert!(
        new_rate * MIN_RUNWAY_SLOTS <= reward_reserves,
        "Reserves must cover runway"
    );
    ...
    assert!(
        result.is_ok(),
        "set_reward_rate should succeed: {:?}",
        result.err()
    );
}

#[test]
fn test_set_reward_rate_exceeds_apr_cap() {
    let total_staked = 100_000_000_000u64;
    let vault_balance = total_staked + 500_000_000_000;

    let (mut svm, admin, channel_config, stake_pool, vault) =
        setup_reward_rate_env(total_staked, vault_balance);

    // Max rate at 15% APR cap
    let max_rate = (total_staked as u128)
    ...
    assert!(
        result.is_err(),
        "set_reward_rate should fail when rate exceeds 15% APR cap"
    );
}

#[test]
fn test_set_reward_rate_insufficient_runway() {
    let total_staked = 100_000_000_000u64;
    let vault_balance = total_staked; // Zero reserves
    ...
    let ix = build_set_reward_rate_ix(
        ...
        &stake_pool,
        &vault,
        1u64, // Any non-zero rate should fail
    );
    ...
    assert!(
        result.is_err(),
        "set_reward_rate should fail with zero reserves"
    );
}
