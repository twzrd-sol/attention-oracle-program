...
pub fn update_pool_rewards(pool: &mut ChannelStakePool, current_slot: u64) -> Result<()> {
    ...
    let rewards_accrued = (pool.reward_per_slot as u128) // SAFE: widening cast
    ...
    let reward_per_share_increase = rewards_accrued
        ...
        .ok_or(OracleError::MathOverflow)?
        .checked_div(pool.total_weighted as u128) // SAFE: widening cast
        .ok_or(OracleError::MathOverflow)?;
...
pub fn calculate_pending_rewards(
    ...
    let weighted_stake = (user_stake.amount as u128) // SAFE: widening cast
        .checked_mul(user_stake.multiplier_bps as u128) // SAFE: widening cast
        .ok_or(OracleError::MathOverflow)?
    ...
    let pending = accumulated
        ...
        .ok_or(OracleError::MathOverflow)?
        .checked_add(user_stake.pending_rewards as u128) // SAFE: widening cast
        .ok_or(OracleError::MathOverflow)?;
...
pub fn calculate_reward_debt(
    ...
    let weighted_stake = (amount as u128) // SAFE: widening cast
...
pub fn stake_channel(ctx: Context<StakeChannel>, amount: u64, lock_duration: u64) -> Result<()> {
    ...
    let weighted_amount = u64::try_from(
        (actual_received as u128) // SAFE: widening cast
            .checked_mul(multiplier_bps as u128) // SAFE: widening cast
            .ok_or(OracleError::MathOverflow)?
            .checked_div(BOOST_PRECISION as u128) // SAFE: widening cast
            .ok_or(OracleError::MathOverflow)?,
