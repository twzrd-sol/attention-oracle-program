...
#[inline(always)] fn read_u8(d: &[u8], o: usize) -> u8 { d[o] }
#[inline(always)] fn read_u16_le(d: &[u8], o: usize) -> u16 { u16::from_le_bytes([d[o], d[o+1]]) }
#[inline(always)] fn read_u32_le(d: &[u8], o: usize) -> u32 {
    let mut b = [0u8; 4]; b.copy_from_slice(&d[o..o+4]); u32::from_le_bytes(b)
}
#[inline(always)] fn read_u64_le(d: &[u8], o: usize) -> u64 {
    let mut b = [0u8; 8]; b.copy_from_slice(&d[o..o+8]); u64::from_le_bytes(b)
}
#[inline(always)] fn read_u128_le(d: &[u8], o: usize) -> u128 {
    let mut b = [0u8; 16]; b.copy_from_slice(&d[o..o+16]); u128::from_le_bytes(b)
}
#[inline(always)] fn read_pubkey(d: &[u8], o: usize) -> Pubkey {
    let mut k = [0u8; 32]; k.copy_from_slice(&d[o..o+32]); k
}
#[inline(always)] fn write_u8(d: &mut [u8], o: usize, v: u8) { d[o] = v; }
#[inline(always)] fn write_u16_le(d: &mut [u8], o: usize, v: u16) {
    d[o..o+2].copy_from_slice(&v.to_le_bytes());
}
#[inline(always)] fn write_u32_le(d: &mut [u8], o: usize, v: u32) {
    d[o..o+4].copy_from_slice(&v.to_le_bytes());
}
#[inline(always)] fn write_u64_le(d: &mut [u8], o: usize, v: u64) {
    d[o..o+8].copy_from_slice(&v.to_le_bytes());
}
#[inline(always)] fn write_u128_le(d: &mut [u8], o: usize, v: u128) {
    d[o..o+16].copy_from_slice(&v.to_le_bytes());
}
#[inline(always)] fn write_pubkey(d: &mut [u8], o: usize, k: &Pubkey) {
...
fn token_owner_key(ai: &AccountInfo) -> Result<Pubkey, ProgramError> {
    let d = unsafe { ai.borrow_data_unchecked() };
    if d.len() < 64 { return Err(ProgramError::InvalidAccountData); }
    Ok(read_pubkey(&d, 32))
}
#[inline(always)]
fn token_mint_key(ai: &AccountInfo) -> Result<Pubkey, ProgramError> {
    let d = unsafe { ai.borrow_data_unchecked() };
    if d.len() < 32 { return Err(ProgramError::InvalidAccountData); }
    Ok(read_pubkey(&d, 0))
}
#[inline(always)]
fn mint_decimals(ai: &AccountInfo) -> Result<u8, ProgramError> {
    let d = unsafe { ai.borrow_data_unchecked() };
    if d.len() < 45 { return Err(ProgramError::InvalidAccountData); }
    Ok(d[44])
}
...
fn verify_legacy_protocol_admin(
    ...
    if d.len() < PROTOCOL_STATE_LEN { return Err(ProgramError::InvalidAccountData); }
    if admin.key() != &read_pubkey(&d, PS_ADMIN) {
        return Err(ProgramError::Custom(ERR_UNAUTHORIZED));
    }
    let bump = read_u8(&d, PS_BUMP);
    let pda = pubkey::create_program_address(
        &[PROTOCOL_SEED, mint.key().as_ref(), &[bump]], program_id)?;
...
fn verify_live_protocol(
    ...
    if !pubkey::pubkey_eq(&pda, protocol_state.key()) { return Err(ProgramError::InvalidSeeds); }
    Ok((m, paused))
}
...
fn verify_stake_pool(
    ...
    if d.len() < CHANNEL_STAKE_POOL_LEN { return Err(ProgramError::InvalidAccountData); }
    if vault.key() != &read_pubkey(&d, SP_VAULT) { return Err(ProgramError::InvalidAccountData); }
    if let Some(m) = mint {
        if m.key() != &read_pubkey(&d, SP_MINT) { return Err(ProgramError::Custom(ERR_INVALID_MINT)); }
    }
    let b = read_u8(&d, SP_BUMP);
    let pda = pubkey::create_program_address(
        &[CHANNEL_STAKE_POOL_SEED, channel_config.key().as_ref(), &[b]], program_id)?;
    if !pubkey::pubkey_eq(&pda, stake_pool.key()) { return Err(ProgramError::InvalidSeeds); }
    Ok(b)
}
...
fn update_pool_rewards(pool: &mut [u8], slot: u64) -> Result<(), ProgramError> {
    ...
    let elapsed = slot.checked_sub(last).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?;
    let rps = read_u64_le(pool, SP_REWARD_PER_SLOT);
    let accrued = (rps as u128).checked_mul(elapsed as u128)
        .ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?;
    let inc = accrued.checked_mul(REWARD_PRECISION)
        .ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?
        .checked_div(tw as u128)
        .ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?;
    let acc = read_u128_le(pool, SP_ACC_REWARD_PER_SHARE);
    write_u128_le(pool, SP_ACC_REWARD_PER_SHARE,
        acc.checked_add(inc).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?);
    write_u64_le(pool, SP_LAST_REWARD_SLOT, slot);
    Ok(())
}
...
fn pending_rewards(us: &[u8], sp: &[u8]) -> Result<u64, ProgramError> {
    ...
    let w = (amt as u128).checked_mul(mul as u128).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?
        .checked_div(BOOST_PRECISION as u128).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?;
    let total = w.checked_mul(acc).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?
        .checked_div(REWARD_PRECISION).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?;
    let p = total.checked_sub(debt).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?
        .checked_add(pend as u128).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?;
...
fn reward_debt(amt: u64, mul: u64, acc: u128) -> Result<u128, ProgramError> {
    let w = (amt as u128).checked_mul(mul as u128).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?
        .checked_div(BOOST_PRECISION as u128).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?;
    w.checked_mul(acc).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?
        .checked_div(REWARD_PRECISION).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))
}
...
fn cpi_transfer_checked(
    tp: &AccountInfo, src: &AccountInfo, mint: &AccountInfo,
    dst: &AccountInfo, auth: &AccountInfo,
    amount: u64, decimals: u8, signers: &[Signer],
) -> ProgramResult {
    let mut data = [0u8; 10];
    data[0] = 12; // TransferChecked
    data[1..9].copy_from_slice(&amount.to_le_bytes());
    data[9] = decimals;
    let m = [AccountMeta::writable(src.key()), AccountMeta::readonly(mint.key()),
             AccountMeta::writable(dst.key()), AccountMeta::readonly_signer(auth.key())];
    let ix = Instruction { program_id: &TOKEN_2022_PROGRAM_ID, accounts: &m, data: &data };
    pinocchio::cpi::slice_invoke_signed(&ix, &[src, mint, dst, auth, tp], signers)
}

#[inline(never)]
fn cpi_mint_to(
    tp: &AccountInfo, mint: &AccountInfo,
    dst: &AccountInfo, auth: &AccountInfo,
    ...
    data[0] = 7; // MintTo
    data[1..9].copy_from_slice(&amount.to_le_bytes());
    let m = [AccountMeta::writable(mint.key()), AccountMeta::writable(dst.key()),
...
#[inline(never)]
fn cpi_burn(
    tp: &AccountInfo, acct: &AccountInfo,
    mint: &AccountInfo, auth: &AccountInfo,
    amount: u64, signers: &[Signer],
) -> ProgramResult {
    let mut data = [0u8; 9];
    data[0] = 8; // Burn
    data[1..9].copy_from_slice(&amount.to_le_bytes());
    let m = [AccountMeta::writable(acct.key()), AccountMeta::writable(mint.key()),
             AccountMeta::readonly_signer(auth.key())];
    let ix = Instruction { program_id: &TOKEN_2022_PROGRAM_ID, accounts: &m, data: &data };
...
#[inline(never)]
fn cpi_create_ata(
    payer: &AccountInfo, owner: &AccountInfo, mint: &AccountInfo,
    tp: &AccountInfo, ata: &AccountInfo,
    sys: &AccountInfo, atp: &AccountInfo,
) -> ProgramResult {
    let m = [
        AccountMeta::writable_signer(payer.key()), AccountMeta::writable(ata.key()),
        AccountMeta::readonly(owner.key()), AccountMeta::readonly(mint.key()),
        AccountMeta::readonly(sys.key()), AccountMeta::readonly(tp.key()),
    ];
    let ix = Instruction { program_id: &ASSOCIATED_TOKEN_PROGRAM_ID, accounts: &m, data: &[1] };
...
fn close_account(acct: &AccountInfo, dest: &AccountInfo) -> ProgramResult {
    ...
    *s = 0;
    ...
    unsafe { acct.assign(&crate::SYSTEM_ID); }
    Ok(())
}
...
pub fn initialize_fee_config(
    ...
    if ix_data.len() < 30 { return Err(ProgramError::Custom(ERR_INVALID_INPUT_LENGTH)); }
    ...
    for i in 0..6 { tier_mults[i] = read_u32_le(ix_data, 6 + i * 4); }

    if treasury_fee_bps > 10_000 || creator_fee_bps > 10_000
    ...
    let (expected, fc_bump) = pubkey::find_program_address(
        &[PROTOCOL_SEED, mint.key().as_ref(), b"fee_config"], program_id);
    ...
    let seeds = [
        Seed::from(PROTOCOL_SEED), Seed::from(mint.key().as_ref()),
        Seed::from(b"fee_config" as &[u8]), Seed::from(bump_ref.as_ref()),
    ];
    let signer = Signer::from(&seeds);

    crate::cpi_create_account(admin, fee_config, Rent::get()?.minimum_balance(FEE_CONFIG_LEN), FEE_CONFIG_LEN as u64, program_id, &[signer])?;

    {
        ...
        for i in 0..6 { write_u32_le(&mut d, FC_TIER_MULTIPLIERS + i*4, tier_mults[i]); }
...
pub fn initialize_stake_pool(
    ...
    // Verify channel_config.mint
    {
        ...
        if d.len() < CHANNEL_CONFIG_V2_LEN { return Err(ProgramError::InvalidAccountData); }
        if read_pubkey(&d, CC_MINT) != ps_mint {
            return Err(ProgramError::Custom(ERR_INVALID_MINT));
        }
    }
    if mint.key() != &ps_mint { return Err(ProgramError::Custom(ERR_INVALID_MINT)); }

    // Derive + create stake_pool
    let (exp_sp, sp_bump) = pubkey::find_program_address(
        &[CHANNEL_STAKE_POOL_SEED, channel_config.key().as_ref()], program_id);
    ...
    let rent = Rent::get()?;
    crate::cpi_create_account(payer, stake_pool, rent.minimum_balance(CHANNEL_STAKE_POOL_LEN), CHANNEL_STAKE_POOL_LEN as u64, program_id, &[sp_signer])?;

    // Derive + create vault token account
    let (exp_v, v_bump) = pubkey::find_program_address(
        &[STAKE_VAULT_SEED, stake_pool.key().as_ref()], program_id);
    ...
    crate::cpi_create_account(payer, vault, rent.minimum_balance(165), 165, &TOKEN_2022_PROGRAM_ID, &[v_signer])?;

    // InitializeAccount3: [18, owner(32)]
    {
        let mut id = [0u8; 33]; id[0] = 18;
        id[1..33].copy_from_slice(stake_pool.key());
        let m = [AccountMeta::writable(vault.key()), AccountMeta::readonly(mint.key())];
        let ix = Instruction { program_id: &TOKEN_2022_PROGRAM_ID, accounts: &m, data: &id };
...
pub fn stake_channel(
    ...
    if accounts.len() < 15 { return Err(ProgramError::NotEnoughAccountKeys); }
    let user = &accounts[0];
    ...
    if ix_data.len() < 16 { return Err(ProgramError::Custom(ERR_INVALID_INPUT_LENGTH)); }

    let amount = read_u64_le(ix_data, 0);
    let lock_duration = read_u64_le(ix_data, 8);
    if amount < MIN_STAKE_AMOUNT { return Err(ProgramError::Custom(ERR_STAKE_BELOW_MINIMUM)); }
    if lock_duration > MAX_LOCK_SLOTS { return Err(ProgramError::Custom(ERR_LOCK_PERIOD_TOO_LONG)); }

    // Verify protocol_state not paused
    let (ps_mint, paused) = verify_live_protocol(user, protocol_state, program_id, false)?;
    if paused { return Err(ProgramError::Custom(ERR_PROTOCOL_PAUSED)); }

    // Verify channel_config + mint
    { let d = unsafe { channel_config.borrow_data_unchecked() };
      if d.len() < CHANNEL_CONFIG_V2_LEN { return Err(ProgramError::InvalidAccountData); }
      if read_pubkey(&d, CC_MINT) != ps_mint { return Err(ProgramError::Custom(ERR_INVALID_MINT)); }
    }
    if mint.key() != &ps_mint { return Err(ProgramError::Custom(ERR_INVALID_MINT)); }
    ...
    if &token_owner_key(user_token_account)? != user.key() { return Err(ProgramError::Custom(ERR_UNAUTHORIZED)); }
    if &token_mint_key(user_token_account)? != mint.key() { return Err(ProgramError::Custom(ERR_INVALID_MINT)); }

    let clock = Clock::get()?;
    let current_slot = clock.slot;
    let multiplier_bps = calculate_boost_bps(lock_duration);
    let lock_end_slot = if lock_duration > 0 {
        current_slot.checked_add(lock_duration).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?
    } else { 0 };
    let decimals = mint_decimals(mint)?;

    // Capture vault balance, transfer, measure actual received
    let vault_before = token_amount(vault)?;
    cpi_transfer_checked(token_program, user_token_account, mint, vault, user,
        amount, decimals, &[])?;
    let actual_received = token_amount(vault)?
        .checked_sub(vault_before).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?;
    if actual_received == 0 { return Err(ProgramError::Custom(ERR_STAKE_BELOW_MINIMUM)); }

    // Handle NFT mint
    let (_, nft_bump) = pubkey::find_program_address(
        &[STAKE_NFT_MINT_SEED, stake_pool.key().as_ref(), user.key().as_ref()], program_id);
    let nft_bump_ref = [nft_bump];
    let pool_bump_ref = [sp_bump];

    if nft_mint.data_len() > 0 {
        // Re-stake: ATA + conditional mint
        cpi_create_ata(payer, user, nft_mint, token_program, nft_ata, system_program, ata_program)?;
        let has_auth = {
            let d = unsafe { nft_mint.borrow_data_unchecked() };
            d.len() >= 36 && read_u32_le(&d, 0) == 1 && &read_pubkey(&d, 4) == stake_pool.key()
        };
        if has_auth {
            let ps = [Seed::from(CHANNEL_STAKE_POOL_SEED),
                       Seed::from(channel_config.key().as_ref()),
        ...
        let rent = Rent::get()?;
        let ns = [Seed::from(STAKE_NFT_MINT_SEED), Seed::from(stake_pool.key().as_ref()),
                   Seed::from(user.key().as_ref()), Seed::from(nft_bump_ref.as_ref())];
        let nsig = Signer::from(&ns);
        crate::cpi_create_account(payer, nft_mint, rent.minimum_balance(170), 170, &TOKEN_2022_PROGRAM_ID, &[nsig])?;

        // InitializeNonTransferableMint (37)
        let m1 = [AccountMeta::writable(nft_mint.key())];
        pinocchio::cpi::slice_invoke_signed(
            &Instruction { program_id: &TOKEN_2022_PROGRAM_ID, accounts: &m1, data: &[37u8] },
            &[nft_mint, token_program], &[])?;

        // InitializeMint2 (20): [20, dec, auth(32), option(1), freeze(32)]
        let mut imd = [0u8; 67]; imd[0] = 20; imd[1] = 0;
        imd[2..34].copy_from_slice(stake_pool.key());
        imd[34] = 1; imd[35..67].copy_from_slice(stake_pool.key());
        let m2 = [AccountMeta::writable(nft_mint.key())];
        pinocchio::cpi::slice_invoke_signed(
            &Instruction { program_id: &TOKEN_2022_PROGRAM_ID, accounts: &m2, data: &imd },
            &[nft_mint, token_program], &[])?;
    ...
    let current_acc = {
        ...
        let acc = read_u128_le(&sp, SP_ACC_REWARD_PER_SHARE);
        let wa = u64::try_from(
            (actual_received as u128).checked_mul(multiplier_bps as u128)
                .ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?
        ...
        write_u64_le(&mut sp, SP_TOTAL_STAKED,
            ts.checked_add(actual_received).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?);
        write_u64_le(&mut sp, SP_TOTAL_WEIGHTED,
            tw.checked_add(wa).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?);
        write_u64_le(&mut sp, SP_STAKER_COUNT,
            sc.checked_add(1).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?);
        acc
    };

    // Create user_stake account
    let (_, us_bump) = pubkey::find_program_address(
        &[CHANNEL_USER_STAKE_SEED, channel_config.key().as_ref(), user.key().as_ref()], program_id);
    let us_bump_ref = [us_bump];
    let us_seeds = [Seed::from(CHANNEL_USER_STAKE_SEED), Seed::from(channel_config.key().as_ref()),
                     Seed::from(user.key().as_ref()), Seed::from(us_bump_ref.as_ref())];
    let us_signer = Signer::from(&us_seeds);
    let rent = Rent::get()?;
    crate::cpi_create_account(payer, user_stake, rent.minimum_balance(USER_CHANNEL_STAKE_LEN), USER_CHANNEL_STAKE_LEN as u64, program_id, &[us_signer])?;
...
pub fn unstake_channel(
    ...
    let (us_amount, us_mul, us_lock, us_nft, us_user, us_bump) = {
        ...
        if d.len() < USER_CHANNEL_STAKE_LEN { return Err(ProgramError::InvalidAccountData); }
        (read_u64_le(&d, US_AMOUNT), read_u64_le(&d, US_MULTIPLIER_BPS),
         read_u64_le(&d, US_LOCK_END_SLOT), read_pubkey(&d, US_NFT_MINT),
         read_pubkey(&d, US_USER), read_u8(&d, US_BUMP))
    };
    if &us_user != user.key() { return Err(ProgramError::Custom(ERR_UNAUTHORIZED)); }
    let pda = pubkey::create_program_address(
        &[CHANNEL_USER_STAKE_SEED, channel_config.key().as_ref(), user.key().as_ref(), &[us_bump]],
        program_id)?;
    if !pubkey::pubkey_eq(&pda, user_stake.key()) { return Err(ProgramError::InvalidSeeds); }
    ...
    if &us_nft != nft_mint.key() { return Err(ProgramError::InvalidAccountData); }
    if &token_owner_key(user_token_account)? != user.key() { return Err(ProgramError::Custom(ERR_UNAUTHORIZED)); }
    if &token_mint_key(user_token_account)? != mint.key() { return Err(ProgramError::Custom(ERR_INVALID_MINT)); }
    ...
    if pend > 0 && !sp_shutdown {
        ...
        let ts = { let d = unsafe { stake_pool.borrow_data_unchecked() }; read_u64_le(&d, SP_TOTAL_STAKED) };
    ...
    let wa = u64::try_from(
        (us_amount as u128).checked_mul(us_mul as u128).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?
            .checked_div(BOOST_PRECISION as u128).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?
    ).map_err(|_| ProgramError::Custom(ERR_MATH_OVERFLOW))?;
    ...
    let ps = [Seed::from(CHANNEL_STAKE_POOL_SEED),
               ...
               Seed::from(pb.as_ref())];
    let psig = Signer::from(&ps);
    cpi_transfer_checked(token_program, vault, mint, user_token_account, stake_pool,
        us_amount, decimals, &[psig])?;
    ...
    // Update pool totals
    {
        ...
        let sc = read_u64_le(&sp, SP_STAKER_COUNT);
        write_u64_le(&mut sp, SP_TOTAL_STAKED,
            ts.checked_sub(us_amount).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?);
        write_u64_le(&mut sp, SP_TOTAL_WEIGHTED,
            tw.checked_sub(wa).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?);
        write_u64_le(&mut sp, SP_STAKER_COUNT,
            sc.checked_sub(1).ok_or(ProgramError::Custom(ERR_MATH_OVERFLOW))?);
    }
...
pub fn claim_channel_rewards(
    ...
    // Verify user_stake
    {
        let d = unsafe { user_stake.borrow_data_unchecked() };
        if d.len() < USER_CHANNEL_STAKE_LEN { return Err(ProgramError::InvalidAccountData); }
        if &read_pubkey(&d, US_USER) != user.key() { return Err(ProgramError::Custom(ERR_UNAUTHORIZED)); }
        let b = read_u8(&d, US_BUMP);
        drop(d);
        let pda = pubkey::create_program_address(
            &[CHANNEL_USER_STAKE_SEED, channel_config.key().as_ref(), user.key().as_ref(), &[b]],
            program_id)?;
        if !pubkey::pubkey_eq(&pda, user_stake.key()) { return Err(ProgramError::InvalidSeeds); }
    }
    ...
    if pend == 0 { return Err(ProgramError::Custom(ERR_NO_REWARDS_TO_CLAIM)); }
