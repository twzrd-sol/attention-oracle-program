...
use anchor_spl::token_interface::{Mint as SplMint, TokenInterface};
...
pub struct InitializeMintOpen<'info> {
    ...
    /// Treasury PDA token account (program-owned, seeds: [b"treasury", mint])
    #[account(
        init,
        payer = admin,
    ...
    /// Creator Pool PDA token account (program-owned, seeds: [b"creator_pool", mint])
    #[account(
        init,
        payer = admin,
        seeds = [b"creator_pool", milo_mint.key().as_ref()],
        bump,
        token::mint = milo_mint,
        token::authority = protocol_state,
    )]
    pub creator_pool: InterfaceAccount<'info, anchor_spl::token_interface::TokenAccount>,
...
pub fn handler_open(
    ...
    protocol_state.creator_pool = ctx.accounts.creator_pool.key();
    protocol_state.mint = ctx.accounts.milo_mint.key();
    protocol_state.paused = false;
    protocol_state.bump = ctx.bumps.protocol_state;
