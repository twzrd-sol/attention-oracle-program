...
// =============================================================================
// REWARD CLAIM HELPER (for emergency unstake)
// =============================================================================
...
pub struct AdminEmergencyUnstake<'info> {
    ...
    /// Oracle protocol state (needed for claim CPI - provides mint reference)
    #[account(
        seeds = [PROTOCOL_SEED, ccm_mint.key().as_ref()],
    ...
    /// Vault's stake position in Oracle
    /// CHECK: Required for emergency unstake and claim
    #[account(mut)]
    pub oracle_user_stake: UncheckedAccount<'info>,
...
pub fn admin_emergency_unstake<'info>(
    ...
    // -------------------------------------------------------------------------
    // Step 1: Claim pending rewards BEFORE emergency unstake
    // -------------------------------------------------------------------------
    ...
    if has_claimable_rewards(
        ...
        msg!("Claiming pending rewards before emergency unstake...");

        let buffer_before_claim = ctx.accounts.vault_ccm_buffer.amount;
    ...
    // -------------------------------------------------------------------------
    // Step 2: Emergency unstake from Oracle (20% penalty)
    // -------------------------------------------------------------------------
    // Capture buffer balance before emergency unstake (after any claim)
    ctx.accounts.vault_ccm_buffer.reload()?;
    ...
    // Reload buffer to see what we received from emergency unstake
    ctx.accounts.vault_ccm_buffer.reload()?;
    ...
    // -------------------------------------------------------------------------
    // All staked CCM + claimed rewards are now pending deposits
    let vault = &mut ctx.accounts.vault;
    vault.total_staked = 0;
    vault.pending_deposits = vault
        .pending_deposits
    ...
    msg!(
        "ADMIN EMERGENCY UNSTAKE: {} CCM was staked, {} returned (Oracle penalty: {}), {} rewards claimed",
        oracle_stake_before,
        ccm_returned,
        oracle_penalty,
        rewards_claimed
