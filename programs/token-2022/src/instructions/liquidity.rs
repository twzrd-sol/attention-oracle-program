    ...
    match tier {
        1 => {
            require!(
                ...
                ProtocolError::DripAlreadyExecuted
            );
            engine.state.tier_1_complete = true;
            engine.state.current_tier = engine.state.current_tier.max(1);
        }
        2 => {
            require!(engine.state.tier_1_complete, ProtocolError::InvalidDripTier);
            require!(
        ...
        3 => {
            require!(engine.state.tier_2_complete, ProtocolError::InvalidDripTier);
            require!(
        ...
        _ => return err!(ProtocolError::InvalidDripTier),
    }
