...
pub fn set_merkle_root_ring(
    ...
    require!(channel_state.version > 0, ProtocolError::ChannelNotInitialized);
    require!(
        channel_state.streamer == streamer_key,
...
pub fn claim_with_ring(
    ...
    require!(!slot.test_bit(index as usize), ProtocolError::AlreadyClaimed);

    // Verify merkle proof (would use actual verification here)
    // For now just a placeholder
    require!(proof.len() > 0, ProtocolError::InvalidProof);
